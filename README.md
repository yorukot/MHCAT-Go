...
- parity-audited warning system with independently gated `/警告紀錄`, `/警告設定`, `/警告清除`, `/警告全部清除`, and `/警告`; exact UI, mixed Mongo rows, threshold actions, staging, and rollback are recorded in [docs/84-warning-system.md](docs/84-warning-system.md).
- parity-audited `/刪除訊息` message cleanup with exact ephemeral UI, legacy permission/count rules, safe Discord pagination/age filtering, and no Mongo dependency; see [docs/85-message-cleanup.md](docs/85-message-cleanup.md).
- parity-audited `/刪除資料` destructive config cleanup command with exact owner-scoped one-hour UI, effective public responses, isolated duplicate cleanup, and disabled-by-default ownership; see the [delete-data parity contract](docs/83-delete-data.md).
...
- parity-audited, gated paid auto-chat MessageCreate handoff with transactional `chatgpt_gets` debit plus rollback-compatible `chatgpts` publication, legacy cooldown/reset timing, and mention-safe worker replies; see [docs/91-autochat-paid.md](docs/91-autochat-paid.md).
- parity-audited, gated `automatic-notification`, `/自動通知列表`, and `/自動通知刪除` setup/list/delete commands plus a separately gated lease-backed recurring delivery worker for legacy `cron_sets` rows; see [docs/92-auto-notification.md](docs/92-auto-notification.md).
...
- parity-audited `/帳號需創建時數` config and member-add policy; exact UI, scalar compatibility, DM/kick/log ordering, intent/ownership boundaries, staging, and rollback are in the [account-age parity contract](docs/79-account-age.md).
- parity-audited `/驗證設置` and `/驗證` captcha/button/modal flow; exact UI, scalar compatibility, atomic challenge state, role/nickname ordering, ownership, staging, and rollback are in the [verification parity contract](docs/80-verification.md).
- parity-audited `/加入訊息設置`, `/退出訊息設置`, generic/special welcome delivery, and leave delivery; exact UI, scalar compatibility, cache-only sends, ownership, staging, and rollback are in the [welcome and leave message parity contract](docs/82-welcome-leave.md).
...
- parity-audited join-role setup/delete plus independently gated `guildMemberAdd` assignment from legacy `join_roles`; exact UI, scalar compatibility, cached role checks, ownership, staging, and rollback are in [docs/81-join-role.md](docs/81-join-role.md).
...
Auto-notification setup/list/delete and delivery are independent, disabled-by-default paths. `MHCAT_FEATURE_AUTO_NOTIFICATION_CONFIG_ENABLED=true` plus `MHCAT_COMMAND_SYNC_INCLUDE_AUTO_NOTIFICATION_CONFIG=true` enables `automatic-notification`, `/自動通知列表`, and `/自動通知刪除`. Recurring sends require `MHCAT_FEATURE_AUTO_NOTIFICATION_DELIVERY_ENABLED=true`, Gateway, scheduler lease enablement, and a non-empty lease owner; the worker uses the `auto-notification-delivery` lease and `Asia/Taipei` schedule time. Disable the Node `handler/cron.js` owner before enabling Go delivery. Exact UI, BSON compatibility, ownership, staging, and rollback are in [docs/92-auto-notification.md](docs/92-auto-notification.md); [docs/66-auto-notification-config.md](docs/66-auto-notification-config.md) remains the implementation overview.

`/兌換` is disabled by default and is available only when paired with `MHCAT_FEATURE_REDEEM_ENABLED=true` and `MHCAT_COMMAND_SYNC_INCLUDE_REDEEM=true` in staging command sync. It consumes one exact fetched `codes` row, replaces one arbitrary matching `chatgpt_gets` row by delete+insert, preserves Mongoose number and duplicate behavior, and remains intentionally non-transactional. It does not enable either auto-chat runtime. Exact UI, partial-progress recovery, staging, and rollback are in [docs/88-redeem.md](docs/88-redeem.md).

`/自動聊天頻道` and `/自動聊天頻道刪除` are disabled by default and require paired `MHCAT_FEATURE_AUTOCHAT_CONFIG_ENABLED=true` and `MHCAT_COMMAND_SYNC_INCLUDE_AUTOCHAT_CONFIG=true` staging flags. They preserve exact public UI, runtime Manage Messages checks, Mongoose channel coercion, and one arbitrary row replacement/deletion without deduplicating `chats`. They do not enable MessageCreate handling. See [docs/89-autochat-config.md](docs/89-autochat-config.md).

The local auto-chat fallback is independently disabled by default. `MHCAT_FEATURE_AUTOCHAT_FALLBACK_ENABLED=true` requires gateway, Guild Messages, and Message Content flags; it reads one arbitrary configured `chats` channel and `chatgpt_gets.price`, then replies from the exact bundled legacy corpus for missing, negative, malformed, or undefined balances. Null/empty, zero, and positive balances remain silent. It writes no Mongo data; see [docs/90-autochat-fallback.md](docs/90-autochat-fallback.md).

The paid auto-chat handoff is separately disabled by default. It requires `MHCAT_FEATURE_AUTOCHAT_PAID_HANDOFF_ENABLED=true`, `MHCAT_AUTOCHAT_PAID_OWNERSHIP_CONFIRMED=true`, gateway, Guild Messages, Message Content, a transaction-capable Mongo deployment, and the compatible external worker. It atomically debits `chatgpt_gets.price` and publishes the legacy `chatgpts` request, then reads the worker response after ten seconds. Do not set the ownership acknowledgment until the Node MessageCreate handler is stopped and worker/schema/duplicate audits are complete; exact eligibility, UI, transactions, worker scalars, staging, and rollback are in [docs/91-autochat-paid.md](docs/91-autochat-paid.md).
...
The `打工系統` command is available only when `MHCAT_FEATURE_WORK_ENABLED=true`. To include it in staging command-sync dry-run/apply, also set `MHCAT_COMMAND_SYNC_INCLUDE_WORK=true`; staging preflight rejects unpaired sync/runtime flags. Command UI, natural/duplicate row behavior, and mixed scalar start/admin arithmetic are parity-audited. Completed-work payout has independent crash-idempotent one-shot and recurring gates and preserves legacy scalar/duplicate selection while adding atomic markers and lease safety. Production ownership remains disabled pending staging smoke and audits; follow [docs/105-work-system.md](docs/105-work-system.md).

The `/警告紀錄` command is available only when `MHCAT_FEATURE_WARNINGS_ENABLED=true`. To include it in staging command-sync dry-run/apply, also set `MHCAT_COMMAND_SYNC_INCLUDE_WARNINGS=true`; staging preflight rejects unpaired sync/runtime flags. This command reads `warndbs` only and, matching legacy despite its advertised permission metadata, does not enforce Manage Messages at runtime.

The `/警告設定` command is available only when `MHCAT_FEATURE_WARNING_SETTINGS_ENABLED=true`. To include it in staging command-sync dry-run/apply, also set `MHCAT_COMMAND_SYNC_INCLUDE_WARNING_SETTINGS=true`; staging preflight and scripts reject unpaired sync/runtime flags. This command writes only `errors_sets.guild`, `ban_count`, and `move`, aligning duplicate rows without creating an index. It does not create warnings, delete messages, kick, ban, or run escalation.

The `/警告清除` and `/警告全部清除` commands are available only when `MHCAT_FEATURE_WARNING_REMOVAL_ENABLED=true`. To include them in staging command-sync dry-run/apply, also set `MHCAT_COMMAND_SYNC_INCLUDE_WARNING_REMOVAL=true`; staging preflight and scripts reject unpaired sync/runtime flags. These commands mutate only `warndbs`, preserve JavaScript splice behavior and legacy public UI, and send best-effort DMs. Remove-all intentionally cleans every duplicate target row.

The `/警告` command is available only when `MHCAT_FEATURE_WARNING_ISSUE_ENABLED=true`. To include it in staging command-sync dry-run/apply, also set `MHCAT_COMMAND_SYNC_INCLUDE_WARNING_ISSUE=true`; staging preflight and scripts reject unpaired sync/runtime flags. It appends raw reasons with Asia/Taipei timestamps, enforces Manage Messages and strict role hierarchy, and evaluates legacy JavaScript-compatible thresholds only for existing warning rows. Exact `停權` bans; every other stored action kicks. Test only against disposable members and rows. The complete warning contract is [docs/84-warning-system.md](docs/84-warning-system.md).

The `/刪除訊息` command is available only when `MHCAT_FEATURE_MESSAGE_CLEANUP_ENABLED=true`. To include it in staging command-sync dry-run/apply, also set `MHCAT_COMMAND_SYNC_INCLUDE_MESSAGE_CLEANUP=true`; staging preflight and scripts reject unpaired sync/runtime flags. It requires Manage Messages, adds Administrator above 200, refuses more than 1000, preserves exact ephemeral UI and filtered-zero behavior, and reports actual confirmed deletions. Go safely scans target-user pages and retains every message older than 14 days. Test only in disposable staging channels; Discord deletion is irreversible and partial batches cannot roll back. See [docs/85-message-cleanup.md](docs/85-message-cleanup.md).

The `/刪除資料` command is available only when `MHCAT_FEATURE_DELETE_DATA_ENABLED=true`. To include it in staging command-sync dry-run/apply, also set `MHCAT_COMMAND_SYNC_INCLUDE_DELETE_DATA=true`; staging preflight and scripts reject unpaired sync/runtime flags. This command requires Manage Messages to create the exact legacy one-hour owner-scoped prompt and deletes every duplicate guild row only from the selected legacy config target. Its select replies are effectively public because legacy deferred publicly, and the repaired stats target uses the actual `numbers` collection. Test only against backed-up disposable staging data. Exact UI, ownership, collection isolation, safety differences, staging, and rollback are in the [delete-data parity contract](docs/83-delete-data.md).

The `/翻譯` command is available only when `MHCAT_FEATURE_TRANSLATE_ENABLED=true`. To include it in staging command-sync dry-run/apply, also set `MHCAT_COMMAND_SYNC_INCLUDE_TRANSLATE=true`; staging preflight rejects unpaired sync/runtime flags. This command preserves the public defer/loading-follow-up/result UI, uses a bounded form-POST Google Translate-compatible provider, does not require Message Content intent, and does not touch Mongo feature data. Exact UI, timing, provider safety, staging, and rollback are in the [translate parity contract](docs/86-translate.md).

The `/查看餘額` command is available only when `MHCAT_FEATURE_BALANCE_QUERY_ENABLED=true`. To include it in staging command-sync dry-run/apply, also set `MHCAT_COMMAND_SYNC_INCLUDE_BALANCE_QUERY=true`; staging preflight rejects unpaired sync/runtime flags. It reads one legacy `chatgpt_gets` row, preserves Mongoose number display and exact ephemeral UI, writes no balance data, and intentionally returns a controlled red backend error instead of a misleading zero. See the [balance-query parity contract](docs/87-balance-query.md).
...
Statistics command UI, Mongoose scalar/whitespace behavior, cached count quirks, channel overwrites, worker lifecycle, ownership, staging, and rollback are canonical in [docs/93-stats.md](docs/93-stats.md).
...
The `/加入身份組設置` and `/加入身份組刪除` commands are available only when `MHCAT_FEATURE_JOIN_ROLE_CONFIG_ENABLED=true`. To include them in staging command-sync dry-run/apply, also set `MHCAT_COMMAND_SYNC_INCLUDE_JOIN_ROLE_CONFIG=true`; staging preflight rejects unpaired flags. Both commands are publicly discoverable, enforce Manage Messages at runtime, and preserve exact legacy UI including `#53FF53`. Automatic assignment is separately gated by `MHCAT_FEATURE_JOIN_ROLE_ASSIGNMENT_ENABLED=true`, Gateway, and Guild Members intent. See [docs/81-join-role.md](docs/81-join-role.md).
...
Exact command UI, Mongo scalar/duplicate rules, generic/special/leave payloads, cache-only channel behavior, event ordering, migration, staging, and rollback are in [docs/82-welcome-leave.md](docs/82-welcome-leave.md).
