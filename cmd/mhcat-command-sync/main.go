...
func expectedStagingCommands(cfg config.CommandSyncConfig) []string {
	...
	if cfg.IncludeEconomyCoinRank {
		expected = append(expected, featureeconomy.CoinRankCommandName)
	}
	if cfg.IncludeEconomyCoinReset {
		expected = append(expected, featureeconomy.CoinResetCommandName)
	}
	if cfg.IncludeEconomyRPS {
		expected = append(expected, featureeconomy.RockPaperScissorsCommandName)
	}
	if cfg.IncludeEconomyGame {
		expected = append(expected, featureeconomy.CoinGameCommandName)
	}
	if cfg.IncludeEconomyShop {
	...
	if cfg.IncludeWarningSettings {
		expected = append(expected, featuremoderation.WarningSettingsCommandName)
	}
	if cfg.IncludeWarningRemoval {
		expected = append(expected, featuremoderation.WarningRemoveCommandName, featuremoderation.WarningRemoveAllCommandName)
	}
	if cfg.IncludeWarningIssue {
	...
	if cfg.IncludeDeleteData {
		expected = append(expected, featuremoderation.DeleteDataCommandName)
	}
	if cfg.IncludeTranslate {
	...
	if cfg.IncludeGachaDraw {
		expected = append(expected, featuregacha.GachaDrawCommandName)
	}
	if cfg.IncludeGachaPrizeCreate {
	...
	if cfg.IncludeStatsRoleCount {
		expected = append(expected, featurestats.StatsRoleCommandName)
	}
	if cfg.IncludeStatsDelete {
	...
	if cfg.IncludeXPRoleConfig {
		expected = append(expected, featurexp.TextXPRewardRoleCommandName, featurexp.VoiceXPRewardRoleCommandName)
	}
	if cfg.IncludeXPProfileDisabled {
	...
	if cfg.IncludeXPReset {
		expected = append(expected, featurexp.XPResetCommandName)
	}
	if cfg.IncludeXPRank {
		expected = append(expected, featurexp.TextXPRankCommandName, featurexp.VoiceXPRankCommandName)
	}
	if cfg.IncludeVoiceRoomConfig {
