...
func TestCommandSyncIncludeGachaDrawRequiresStagingMode(t *testing.T) {
	...
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include gacha draw performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeGachaDrawStagingDryRunIncludesDefinition(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_GACHA_DRAW"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected gacha draw dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	if !strings.Contains(stdout, "扭蛋") {
		t.Fatalf("gacha draw command missing from dry-run output: %q", stdout)
	}
}
...
func TestCommandSyncIncludeGachaPrizeCreateRequiresStagingMode(t *testing.T) {
	...
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include gacha prize-create performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeGachaPrizeCreateStagingDryRunIncludesDefinition(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_CREATE"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected gacha prize-create dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	if !strings.Contains(stdout, "扭蛋獎池增加") {
		t.Fatalf("gacha prize-create command missing from dry-run output: %q", stdout)
	}
}
...
func TestCommandSyncIncludeGachaPrizeEditRequiresStagingMode(t *testing.T) {
	...
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include gacha prize-edit performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeGachaPrizeEditStagingDryRunIncludesDefinition(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_EDIT"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected gacha prize-edit dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	if !strings.Contains(stdout, "扭蛋獎品編輯") {
		t.Fatalf("gacha prize-edit command missing from dry-run output: %q", stdout)
	}
}
...
func TestCommandSyncIncludeStatsDeleteRequiresStagingMode(t *testing.T) {
	...
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include stats delete performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeStatsDeleteStagingDryRunIncludesDefinition(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_STATS_DELETE"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected stats delete dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	if !strings.Contains(stdout, "統計系統刪除") {
		t.Fatalf("stats delete command missing from dry-run output: %q", stdout)
	}
}
...
func TestCommandSyncIncludeStatsRoleCountRequiresStagingMode(t *testing.T) {
	...
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include stats role-count performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeStatsRoleCountStagingDryRunIncludesDefinition(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_STATS_ROLE_COUNT"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected stats role-count dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	if !strings.Contains(stdout, "統計身分組人數") {
		t.Fatalf("stats role-count command missing from dry-run output: %q", stdout)
	}
}
...
func TestCommandSyncIncludeXPRoleConfigRequiresStagingMode(t *testing.T) {
	...
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include XP role config performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeXPRoleConfigStagingDryRunIncludesDefinitions(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_XP_ROLE_CONFIG"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected XP role config dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	if !strings.Contains(stdout, "聊天經驗身分組設定") || !strings.Contains(stdout, "語音經驗身分組設定") {
		t.Fatalf("XP role config commands missing from dry-run output: %q", stdout)
	}
}
...
func TestCommandSyncIncludeXPAdminRequiresStagingMode(t *testing.T) {
	...
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include XP admin performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeXPAdminStagingDryRunIncludesDefinition(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_XP_ADMIN"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected XP admin dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	if !strings.Contains(stdout, "經驗值改變") {
		t.Fatalf("XP admin command missing from dry-run output: %q", stdout)
	}
}
...
func TestCommandSyncIncludeXPResetRequiresStagingMode(t *testing.T) {
	...
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include XP reset performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeXPResetStagingDryRunIncludesDefinition(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_XP_RESET"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected XP reset dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	if !strings.Contains(stdout, "經驗值重製") {
		t.Fatalf("XP reset command missing from dry-run output: %q", stdout)
	}
}
...
func TestCommandSyncIncludeXPRankRequiresStagingMode(t *testing.T) {
	...
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include XP rank performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeXPRankStagingDryRunIncludesDefinitions(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_XP_RANK"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected XP rank dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	for _, name := range []string{"聊天排行榜", "語音排行榜"} {
		if !strings.Contains(stdout, name) {
			t.Fatalf("XP rank command %q missing from dry-run output: %q", name, stdout)
		}
	}
}
...
func TestCommandSyncIncludeVoiceRoomLockRequiresStagingMode(t *testing.T) {
	...
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include voice-room lock performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeVoiceRoomLockStagingDryRunIncludesDefinitions(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_VOICE_ROOM_LOCK"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected voice-room lock dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	if !strings.Contains(stdout, "上鎖頻道") {
		t.Fatalf("voice-room lock command missing from dry-run output: %q", stdout)
	}
}
...
func TestCommandSyncIncludeEconomyCoinResetRequiresStagingMode(t *testing.T) {
	env := baseCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_COIN_RESET"] = "true"
	exitCode, _, stderr, fake := runWithFakeClient(t, nil, env, &fakediscord.CommandSyncClient{}, defaultCommandRegistry)
	if exitCode == 0 {
		t.Fatal("expected include economy coin-reset without staging mode to fail")
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include economy coin-reset performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeEconomyCoinResetStagingDryRunIncludesDefinition(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_COIN_RESET"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected economy coin-reset dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	if !strings.Contains(stdout, "代幣重製") {
		t.Fatalf("economy coin-reset command missing from dry-run output: %q", stdout)
	}
}
...
func TestCommandSyncIncludeEconomyRPSRequiresStagingMode(t *testing.T) {
	...
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include economy RPS performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeEconomyRPSStagingDryRunIncludesDefinition(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_RPS"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected economy RPS dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	if !strings.Contains(stdout, "剪刀石頭布") {
		t.Fatalf("economy RPS command missing from dry-run output: %q", stdout)
	}
}
...
func TestCommandSyncIncludeEconomyGameRequiresStagingMode(t *testing.T) {
	...
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include economy game performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeEconomyGameStagingDryRunIncludesDefinition(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_GAME"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected economy game dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	if !strings.Contains(stdout, "代幣遊戲") {
		t.Fatalf("economy game command missing from dry-run output: %q", stdout)
	}
}
...
func TestCommandSyncIncludeEconomyShopRequiresStagingMode(t *testing.T) {
	...
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("unsafe include economy shop performed writes: %#v", fake)
	}
	if !strings.Contains(stderr, "MHCAT_STAGING_MODE") {
		t.Fatalf("expected staging mode error, stderr=%q", stderr)
	}
...
func TestCommandSyncIncludeEconomyShopStagingDryRunIncludesDefinition(t *testing.T) {
	env := stagingCommandSyncEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_SHOP"] = "true"
	fake := &fakediscord.CommandSyncClient{}
	exitCode, stdout, stderr, _ := runWithFakeClient(t, nil, env, fake, defaultCommandRegistry)
	if exitCode != 0 {
		t.Fatalf("expected economy shop dry-run to pass, stderr=%q stdout=%q", stderr, stdout)
	}
	if len(fake.Created) != 0 || len(fake.Updated) != 0 || len(fake.Deleted) != 0 {
		t.Fatalf("dry-run performed writes: %#v", fake)
	}
	if !strings.Contains(stdout, "代幣商店") {
		t.Fatalf("economy shop command missing from dry-run output: %q", stdout)
	}
}
