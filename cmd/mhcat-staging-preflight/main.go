...
func buildReport(lookup lookupFunc) []checkResult {
	checks := []checkResult{
		...
		gachaDrawCommandSync(lookup),
		gachaDrawRuntimePairing(lookup),
		gachaPrizeCreateCommandSync(lookup),
		gachaPrizeCreateRuntimePairing(lookup),
		gachaPrizeEditCommandSync(lookup),
		gachaPrizeEditRuntimePairing(lookup),
...
func warningIssueCommandSync(lookup lookupFunc) checkResult {
	includeWarningIssue, err := boolValue(lookup, "MHCAT_COMMAND_SYNC_INCLUDE_WARNING_ISSUE")
	if err != nil {
		return checkResult{Name: "warning-issue-command-sync", Status: statusFail, Message: err.Error()}
	}
	if includeWarningIssue {
		return checkResult{Name: "warning-issue-command-sync", Status: statusPass, Message: "warning issue command sync include is enabled for staging review"}
	}
	return checkResult{Name: "warning-issue-command-sync", Status: statusSkipped, Message: "warning issue command sync include is disabled"}
}

func warningIssueRuntimePairing(lookup lookupFunc) checkResult {
	includeWarningIssue, err := boolValue(lookup, "MHCAT_COMMAND_SYNC_INCLUDE_WARNING_ISSUE")
	if err != nil {
		return checkResult{Name: "warning-issue-runtime-pairing", Status: statusFail, Message: err.Error()}
	}
	warningIssueEnabled, err := boolValue(lookup, "MHCAT_FEATURE_WARNING_ISSUE_ENABLED")
	if err != nil {
		return checkResult{Name: "warning-issue-runtime-pairing", Status: statusFail, Message: err.Error()}
	}
	if includeWarningIssue && !warningIssueEnabled {
		return checkResult{Name: "warning-issue-runtime-pairing", Status: statusFail, Message: "MHCAT_COMMAND_SYNC_INCLUDE_WARNING_ISSUE=true requires MHCAT_FEATURE_WARNING_ISSUE_ENABLED=true in the staging runtime"}
	}
	if includeWarningIssue && warningIssueEnabled {
		return checkResult{Name: "warning-issue-runtime-pairing", Status: statusPass, Message: "warning issue command sync and runtime feature flag are paired"}
	}
	if warningIssueEnabled {
...
func gachaDrawCommandSync(lookup lookupFunc) checkResult {
	includeGacha, err := boolValue(lookup, "MHCAT_COMMAND_SYNC_INCLUDE_GACHA_DRAW")
	if err != nil {
		return checkResult{Name: "gacha-draw-command-sync", Status: statusFail, Message: err.Error()}
	}
	if includeGacha {
		return checkResult{Name: "gacha-draw-command-sync", Status: statusPass, Message: "gacha draw command sync include is enabled for staging review"}
	}
	return checkResult{Name: "gacha-draw-command-sync", Status: statusSkipped, Message: "gacha draw command sync include is disabled"}
}

func gachaDrawRuntimePairing(lookup lookupFunc) checkResult {
	includeGacha, err := boolValue(lookup, "MHCAT_COMMAND_SYNC_INCLUDE_GACHA_DRAW")
	if err != nil {
		return checkResult{Name: "gacha-draw-runtime-pairing", Status: statusFail, Message: err.Error()}
	}
	gachaEnabled, err := boolValue(lookup, "MHCAT_FEATURE_GACHA_DRAW_ENABLED")
	if err != nil {
		return checkResult{Name: "gacha-draw-runtime-pairing", Status: statusFail, Message: err.Error()}
	}
	if includeGacha && !gachaEnabled {
		return checkResult{Name: "gacha-draw-runtime-pairing", Status: statusFail, Message: "MHCAT_COMMAND_SYNC_INCLUDE_GACHA_DRAW=true requires MHCAT_FEATURE_GACHA_DRAW_ENABLED=true in the staging runtime"}
	}
	if includeGacha && gachaEnabled {
		return checkResult{Name: "gacha-draw-runtime-pairing", Status: statusPass, Message: "gacha draw command sync and runtime feature flag are paired"}
	}
	if gachaEnabled {
		return checkResult{Name: "gacha-draw-runtime-pairing", Status: statusWarn, Message: "gacha draw runtime is enabled but command sync include is disabled"}
	}
	return checkResult{Name: "gacha-draw-runtime-pairing", Status: statusSkipped, Message: "gacha draw runtime and command sync include are disabled"}
}
...
func gachaPrizeCreateCommandSync(lookup lookupFunc) checkResult {
	includeGacha, err := boolValue(lookup, "MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_CREATE")
	if err != nil {
		return checkResult{Name: "gacha-prize-create-command-sync", Status: statusFail, Message: err.Error()}
	}
	if includeGacha {
		return checkResult{Name: "gacha-prize-create-command-sync", Status: statusPass, Message: "gacha prize-create command sync include is enabled for staging review"}
	}
	return checkResult{Name: "gacha-prize-create-command-sync", Status: statusSkipped, Message: "gacha prize-create command sync include is disabled"}
}

func gachaPrizeCreateRuntimePairing(lookup lookupFunc) checkResult {
	includeGacha, err := boolValue(lookup, "MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_CREATE")
	if err != nil {
		return checkResult{Name: "gacha-prize-create-runtime-pairing", Status: statusFail, Message: err.Error()}
	}
	gachaEnabled, err := boolValue(lookup, "MHCAT_FEATURE_GACHA_PRIZE_CREATE_ENABLED")
	if err != nil {
		return checkResult{Name: "gacha-prize-create-runtime-pairing", Status: statusFail, Message: err.Error()}
	}
	if includeGacha && !gachaEnabled {
		return checkResult{Name: "gacha-prize-create-runtime-pairing", Status: statusFail, Message: "MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_CREATE=true requires MHCAT_FEATURE_GACHA_PRIZE_CREATE_ENABLED=true in the staging runtime"}
	}
	if includeGacha && gachaEnabled {
		return checkResult{Name: "gacha-prize-create-runtime-pairing", Status: statusPass, Message: "gacha prize-create command sync and runtime feature flag are paired"}
	}
	if gachaEnabled {
		return checkResult{Name: "gacha-prize-create-runtime-pairing", Status: statusWarn, Message: "gacha prize-create runtime is enabled but command sync include is disabled"}
	}
	return checkResult{Name: "gacha-prize-create-runtime-pairing", Status: statusSkipped, Message: "gacha prize-create runtime and command sync include are disabled"}
}

func gachaPrizeEditCommandSync(lookup lookupFunc) checkResult {
	includeGacha, err := boolValue(lookup, "MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_EDIT")
	if err != nil {
		return checkResult{Name: "gacha-prize-edit-command-sync", Status: statusFail, Message: err.Error()}
	}
	if includeGacha {
		return checkResult{Name: "gacha-prize-edit-command-sync", Status: statusPass, Message: "gacha prize-edit command sync include is enabled for staging review"}
	}
	return checkResult{Name: "gacha-prize-edit-command-sync", Status: statusSkipped, Message: "gacha prize-edit command sync include is disabled"}
}

func gachaPrizeEditRuntimePairing(lookup lookupFunc) checkResult {
	includeGacha, err := boolValue(lookup, "MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_EDIT")
	if err != nil {
		return checkResult{Name: "gacha-prize-edit-runtime-pairing", Status: statusFail, Message: err.Error()}
	}
	gachaEnabled, err := boolValue(lookup, "MHCAT_FEATURE_GACHA_PRIZE_EDIT_ENABLED")
	if err != nil {
		return checkResult{Name: "gacha-prize-edit-runtime-pairing", Status: statusFail, Message: err.Error()}
	}
	if includeGacha && !gachaEnabled {
		return checkResult{Name: "gacha-prize-edit-runtime-pairing", Status: statusFail, Message: "MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_EDIT=true requires MHCAT_FEATURE_GACHA_PRIZE_EDIT_ENABLED=true in the staging runtime"}
	}
	if includeGacha && gachaEnabled {
		return checkResult{Name: "gacha-prize-edit-runtime-pairing", Status: statusPass, Message: "gacha prize-edit command sync and runtime feature flag are paired"}
	}
	if gachaEnabled {
		return checkResult{Name: "gacha-prize-edit-runtime-pairing", Status: statusWarn, Message: "gacha prize-edit runtime is enabled but command sync include is disabled"}
	}
	return checkResult{Name: "gacha-prize-edit-runtime-pairing", Status: statusSkipped, Message: "gacha prize-edit runtime and command sync include are disabled"}
}
...
func statsDeleteCommandSync(lookup lookupFunc) checkResult {
	includeStats, err := boolValue(lookup, "MHCAT_COMMAND_SYNC_INCLUDE_STATS_DELETE")
	if err != nil {
		return checkResult{Name: "stats-delete-command-sync", Status: statusFail, Message: err.Error()}
	}
	if includeStats {
...
func statsDeleteRuntimePairing(lookup lookupFunc) checkResult {
	includeStats, err := boolValue(lookup, "MHCAT_COMMAND_SYNC_INCLUDE_STATS_DELETE")
	if err != nil {
		return checkResult{Name: "stats-delete-runtime-pairing", Status: statusFail, Message: err.Error()}
	}
	statsEnabled, err := boolValue(lookup, "MHCAT_FEATURE_STATS_DELETE_ENABLED")
	if err != nil {
		return checkResult{Name: "stats-delete-runtime-pairing", Status: statusFail, Message: err.Error()}
	}
	if includeStats && !statsEnabled {
		return checkResult{Name: "stats-delete-runtime-pairing", Status: statusFail, Message: "MHCAT_COMMAND_SYNC_INCLUDE_STATS_DELETE=true requires MHCAT_FEATURE_STATS_DELETE_ENABLED=true in the staging runtime"}
	}
	if includeStats && statsEnabled {
		return checkResult{Name: "stats-delete-runtime-pairing", Status: statusPass, Message: "stats delete command sync and runtime feature flag are paired"}
	}
	if statsEnabled {
		return checkResult{Name: "stats-delete-runtime-pairing", Status: statusWarn, Message: "stats delete runtime is enabled but command sync include is disabled"}
	}
	return checkResult{Name: "stats-delete-runtime-pairing", Status: statusSkipped, Message: "stats delete runtime and command sync include are disabled"}
}
