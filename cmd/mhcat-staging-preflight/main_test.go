...
func TestPreflightAcceptsMessageContentForAutoChatFallback(t *testing.T) {
	...
	if !strings.Contains(stdout, "message-content-intent status=pass") || !strings.Contains(stdout, "autochat-fallback-runtime-readiness status=pass") {
		t.Fatalf("expected autochat fallback message content pass checks, stdout=%q", stdout)
	}
}

func TestPreflightRejectsAutoChatFallbackWithoutRequiredGatewayFlags(t *testing.T) {
	for key, want := range map[string]string{
		...
		if !strings.Contains(stdout, "autochat-fallback-runtime-readiness status=fail") || !strings.Contains(stdout, want) {
			t.Fatalf("expected autochat fallback readiness failure %q, stdout=%q", want, stdout)
		}
	}
}
...
func TestPreflightRejectsEconomyCoinAdminCommandSyncWithoutRuntimeFlag(t *testing.T) {
	env := validEnv()
	...
	if !strings.Contains(stdout, "economy-coin-admin-runtime-pairing status=fail") {
		t.Fatalf("expected economy coin-admin pairing failure, stdout=%q", stdout)
	}
}

func TestPreflightAcceptsEconomyCoinAdminCommandSyncWithRuntimeFlag(t *testing.T) {
	env := validEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_COIN_ADMIN"] = "true"
	env["MHCAT_FEATURE_ECONOMY_COIN_ADMIN_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
		t.Fatalf("expected exit 0, stderr=%q stdout=%q", stderr, stdout)
	}
	if !strings.Contains(stdout, "economy-coin-admin-command-sync status=pass") || !strings.Contains(stdout, "economy-coin-admin-runtime-pairing status=pass") {
		t.Fatalf("expected economy coin-admin pass checks, stdout=%q", stdout)
	}
}

func TestPreflightWarnsWhenEconomyCoinAdminRuntimeEnabledWithoutCommandSync(t *testing.T) {
	env := validEnv()
	env["MHCAT_FEATURE_ECONOMY_COIN_ADMIN_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
	...
	if !strings.Contains(stdout, "economy-coin-admin-runtime-pairing status=warn") {
		t.Fatalf("expected economy coin-admin runtime warning, stdout=%q", stdout)
	}
}
...
func TestPreflightRejectsEconomyShopCommandSyncWithoutRuntimeFlag(t *testing.T) {
	...
	if !strings.Contains(stdout, "economy-shop-runtime-pairing status=fail") {
		t.Fatalf("expected economy shop pairing failure, stdout=%q", stdout)
	}
}

func TestPreflightAcceptsEconomyShopCommandSyncWithRuntimeFlag(t *testing.T) {
	env := validEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_SHOP"] = "true"
	env["MHCAT_FEATURE_ECONOMY_SHOP_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
		t.Fatalf("expected exit 0, stderr=%q stdout=%q", stderr, stdout)
	}
	if !strings.Contains(stdout, "economy-shop-command-sync status=pass") || !strings.Contains(stdout, "economy-shop-runtime-pairing status=pass") {
		t.Fatalf("expected economy shop pass checks, stdout=%q", stdout)
	}
}

func TestPreflightWarnsWhenEconomyShopRuntimeEnabledWithoutCommandSync(t *testing.T) {
	env := validEnv()
	env["MHCAT_FEATURE_ECONOMY_SHOP_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
	...
	if !strings.Contains(stdout, "economy-shop-runtime-pairing status=warn") {
		t.Fatalf("expected economy shop runtime warning, stdout=%q", stdout)
	}
}
...
func TestPreflightRejectsEconomyProfileCommandSyncWithoutRuntimeFlag(t *testing.T) {
	...
	if !strings.Contains(stdout, "economy-profile-runtime-pairing status=fail") {
		t.Fatalf("expected economy profile pairing failure, stdout=%q", stdout)
	}
}

func TestPreflightAcceptsEconomyProfileCommandSyncWithRuntimeFlag(t *testing.T) {
	env := validEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_PROFILE"] = "true"
	env["MHCAT_FEATURE_ECONOMY_PROFILE_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
		t.Fatalf("expected exit 0, stderr=%q stdout=%q", stderr, stdout)
	}
	if !strings.Contains(stdout, "economy-profile-command-sync status=pass") || !strings.Contains(stdout, "economy-profile-runtime-pairing status=pass") {
		t.Fatalf("expected economy profile pass checks, stdout=%q", stdout)
	}
}

func TestPreflightWarnsWhenEconomyProfileRuntimeEnabledWithoutCommandSync(t *testing.T) {
	env := validEnv()
	env["MHCAT_FEATURE_ECONOMY_PROFILE_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
	...
	if !strings.Contains(stdout, "economy-profile-runtime-pairing status=warn") {
		t.Fatalf("expected economy profile runtime warning, stdout=%q", stdout)
	}
}
...
func TestPreflightRejectsDeleteDataCommandSyncWithoutRuntimeFlag(t *testing.T) {
	env := validEnv()
	...
	if !strings.Contains(stdout, "delete-data-runtime-pairing status=fail") {
		t.Fatalf("expected delete data pairing failure, stdout=%q", stdout)
	}
}

func TestPreflightAcceptsDeleteDataCommandSyncWithRuntimeFlag(t *testing.T) {
	env := validEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_DELETE_DATA"] = "true"
	env["MHCAT_FEATURE_DELETE_DATA_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
		t.Fatalf("expected exit 0, stderr=%q stdout=%q", stderr, stdout)
	}
	if !strings.Contains(stdout, "delete-data-command-sync status=pass") || !strings.Contains(stdout, "delete-data-runtime-pairing status=pass") {
		t.Fatalf("expected delete data pass checks, stdout=%q", stdout)
	}
}

func TestPreflightWarnsWhenDeleteDataRuntimeEnabledWithoutCommandSync(t *testing.T) {
	env := validEnv()
	env["MHCAT_FEATURE_DELETE_DATA_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
	...
	if !strings.Contains(stdout, "delete-data-runtime-pairing status=warn") {
		t.Fatalf("expected delete data runtime warning, stdout=%q", stdout)
	}
}
...
func TestPreflightRejectsGachaPrizeCreateCommandSyncWithoutRuntimeFlag(t *testing.T) {
	env := validEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_CREATE"] = "true"
	code, stdout, _ := runPreflight(t, nil, env)
	if code == 0 {
		t.Fatal("expected non-zero exit")
	}
	if !strings.Contains(stdout, "gacha-prize-create-runtime-pairing status=fail") {
		t.Fatalf("expected gacha prize-create pairing failure, stdout=%q", stdout)
	}
}

func TestPreflightAcceptsGachaPrizeCreateCommandSyncWithRuntimeFlag(t *testing.T) {
	env := validEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_CREATE"] = "true"
	env["MHCAT_FEATURE_GACHA_PRIZE_CREATE_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
		t.Fatalf("expected exit 0, stderr=%q stdout=%q", stderr, stdout)
	}
	if !strings.Contains(stdout, "gacha-prize-create-command-sync status=pass") || !strings.Contains(stdout, "gacha-prize-create-runtime-pairing status=pass") {
		t.Fatalf("expected gacha prize-create pass checks, stdout=%q", stdout)
	}
}

func TestPreflightWarnsWhenGachaPrizeCreateRuntimeEnabledWithoutCommandSync(t *testing.T) {
	env := validEnv()
	env["MHCAT_FEATURE_GACHA_PRIZE_CREATE_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
	...
	if !strings.Contains(stdout, "gacha-prize-create-runtime-pairing status=warn") {
		t.Fatalf("expected gacha prize-create runtime warning, stdout=%q", stdout)
	}
}
...
func TestPreflightRejectsStatsCreateCommandSyncWithoutRuntimeFlag(t *testing.T) {
	env := validEnv()
	...
	if !strings.Contains(stdout, "stats-create-runtime-pairing status=fail") {
		t.Fatalf("expected stats create pairing failure, stdout=%q", stdout)
	}
}

func TestPreflightAcceptsStatsCreateCommandSyncWithRuntimeFlag(t *testing.T) {
	env := validEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_STATS_CREATE"] = "true"
	env["MHCAT_FEATURE_STATS_CREATE_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
		t.Fatalf("expected exit 0, stderr=%q stdout=%q", stderr, stdout)
	}
	if !strings.Contains(stdout, "stats-create-command-sync status=pass") || !strings.Contains(stdout, "stats-create-runtime-pairing status=pass") {
		t.Fatalf("expected stats create pass checks, stdout=%q", stdout)
	}
}

func TestPreflightWarnsWhenStatsCreateRuntimeEnabledWithoutCommandSync(t *testing.T) {
	env := validEnv()
	env["MHCAT_FEATURE_STATS_CREATE_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
	...
	if !strings.Contains(stdout, "stats-create-runtime-pairing status=warn") {
		t.Fatalf("expected stats create runtime warning, stdout=%q", stdout)
	}
}
...
func TestPreflightRejectsXPRankCommandSyncWithoutRuntimeFlag(t *testing.T) {
	env := validEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_XP_RANK"] = "true"
	code, stdout, _ := runPreflight(t, nil, env)
	if code == 0 {
		t.Fatal("expected non-zero exit")
	}
	if !strings.Contains(stdout, "xp-rank-runtime-pairing status=fail") {
		t.Fatalf("expected XP rank pairing failure, stdout=%q", stdout)
	}
}

func TestPreflightAcceptsXPRankCommandSyncWithRuntimeFlag(t *testing.T) {
	env := validEnv()
	env["MHCAT_COMMAND_SYNC_INCLUDE_XP_RANK"] = "true"
	env["MHCAT_FEATURE_XP_RANK_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
		t.Fatalf("expected exit 0, stderr=%q stdout=%q", stderr, stdout)
	}
	if !strings.Contains(stdout, "xp-rank-command-sync status=pass") || !strings.Contains(stdout, "xp-rank-runtime-pairing status=pass") {
		t.Fatalf("expected XP rank pass checks, stdout=%q", stdout)
	}
}

func TestPreflightWarnsWhenXPRankRuntimeEnabledWithoutCommandSync(t *testing.T) {
	env := validEnv()
	env["MHCAT_FEATURE_XP_RANK_ENABLED"] = "true"
	code, stdout, stderr := runPreflight(t, nil, env)
	if code != 0 {
	...
	if !strings.Contains(stdout, "xp-rank-runtime-pairing status=warn") {
		t.Fatalf("expected XP rank runtime warning, stdout=%q", stdout)
	}
}
