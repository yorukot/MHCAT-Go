...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 群組防護 | `詐騙網址回報`, `帳號需創建時數`, `防詐騙網址` | URLs, hour count, channel/delete toggles | public or `KickMembers`/`ManageMessages` | mostly 10s metadata | `not_a_good_web`, `create_hours`, `good_web` | report webhook, join kick, message delete | defer/edit | user-controlled regex risk; cooldown not enforced | in-progress: `/帳號需創建時數` config, member-add account-age gate, and config-only `/防詐騙網址` toggle implemented behind explicit flags; URL reporting/deletion runtime not-started |
