...
| 生日系統 | `生日系統` subcommands | birthday date/time, message/channel/role settings | config/admin paths `ManageMessages` | 5s metadata | `birthday`, `birthday_set` | intended birthday messages/roles | defer/select/edit | sender/role scheduler commented out | in-progress: gated `祝福語設定`; date/profile flows and scheduler not-started |
