...
| 生日系統 | `生日系統` subcommands | birthday date/time, message/channel/role settings | config/admin paths `ManageMessages` | 5s metadata | `birthday`, `birthday_set` | intended birthday messages/roles | defer/select/edit | sender/role scheduler commented out; legacy allow-admin path forgot to save its replacement row | in-progress: gated `祝福語設定`, `是否允許管理員設定`, `刪除`, and `生日列表`; date add select flow and scheduler not-started |
