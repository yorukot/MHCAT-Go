...
| `good_web.js` | `good_web` | `good_webs` | `guild`, `open` | anti-scam toggle by guild; gated `/防詐騙網址` reads and writes by guild | none | `{guild:1}` candidate only after duplicate audit | boolean toggle; duplicates may exist because legacy delete+insert was not atomic | anti-scam config; Go updates all duplicate guild rows with `$set.open` and only upserts when no rows match; no index is created by startup |
