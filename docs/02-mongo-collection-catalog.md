...
| `cron_set.js` | `cron_set` | `cron_sets` | `cron`, `guild`, `channel`, `id`, `message` | scheduled job by guild/id; gated `/自動通知列表` lists by guild and gated `/自動通知刪除` deletes by `{guild,id}` | none | `{guild:1,id:1}`, `{guild:1}` | `message` is loose Discord payload; `cron:null` or missing `cron` is an abandoned setup draft | auto-notification config list/delete; Go filters and cleans pending drafts, creates no indexes, and does not own scheduler sends |
