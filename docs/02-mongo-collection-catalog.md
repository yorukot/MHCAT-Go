...
| --- | --- | --- | --- | --- |
| `mhcat_scheduler_locks` | single-owner lease primitive used by automatic-notification delivery and one-shot work payout | `_id` equal to `lock_name`, `lock_name`, `owner`, `fence`, `expires_at`, `created_at`, `updated_at` | default Mongo `_id` only | no Node dependency; no backfill; graceful shutdown releases held leases and crashes rely on natural expiry for rollback |
