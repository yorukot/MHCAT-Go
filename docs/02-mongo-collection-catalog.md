...
| `chatgpt_get.js` | `chatgpt_get` | `chatgpt_gets` | `guild`, `price` | guild price/config | balance query reads one arbitrary match; redeem credits; paid runtime transactionally debits and fails closed on duplicates | `{guild:1}` | Mongoose Number coercion; duplicate policy differs by owner; see [87-balance-query.md](87-balance-query.md) | chatbot/economy config |
