...
| `birthday.js` | `birthday` | `birthdays` | `guild`, `user`, birthday date/time numbers, `allow` | user birthday by guild/user; list by guild | none | `{guild:1,user:1}`, `{guild:1}` | scheduler inactive; missing date parts | birthday profile CRUD |
| `birthday_set.js` | `birthday_set` | `birthday_sets` | `guild`, `msg`, `utc`, `channel`, `everyone_can_set_birthday_date`, `role` | guild singleton config | none | `{guild:1}` audit singleton before unique | timezone and role optional | birthday config CRUD |
...
| `chat.js` | `chat` | `chats` | `guild`, `channel` | autochat config by guild/channel | none | `{guild:1}` | MessageContent dependency | chat config CRUD |
