...
| `good_web.js` | `good_web` | `good_webs` | `guild`, `open` | anti-scam toggle by guild; gated `/防詐騙網址` reads/writes by guild and message-delete runtime reads `open` | none | `{guild:1}` candidate only after duplicate audit | boolean toggle; duplicates may exist because legacy delete+insert was not atomic | anti-scam config; Go updates all duplicate guild rows with `$set.open`, only upserts when no rows match, and creates no index at startup |
...
| `not_a_good_web.js` | `not_a_good_web` | `not_a_good_webs` | `web` | scam URL lookup/insert; gated report command and message-delete runtime read existing rows | none | `{web:1}` | regex injection; domain normalization | scam URL catalog; Go report/runtime paths read only and do not write new rows |
