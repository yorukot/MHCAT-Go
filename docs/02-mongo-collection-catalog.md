...
| `chatgpt.js` | `chatgpt` | `chatgpts` | `guild`, `resid_c`, `resid_p`, `reply`, `message`, `time` | chatbot state/handoff by guild | paid runtime transactionally patches request fields and reads the exact-time response | `{guild:1}` | external worker deployment unknown; duplicate guild rows fail closed | legacy worker handoff preserved; deterministic `_id` only for missing rows |
| `chatgpt_get.js` | `chatgpt_get` | `chatgpt_gets` | `guild`, `price` | guild price/config | balance query reads one arbitrary match; redeem replaces one arbitrary match by delete+insert; paid runtime transactionally debits and fails closed on duplicates | none until all writers are audited | Mongoose Number coercion; owner-specific duplicate policy; see [87-balance-query.md](87-balance-query.md) and [88-redeem.md](88-redeem.md) | chatbot/economy config |
