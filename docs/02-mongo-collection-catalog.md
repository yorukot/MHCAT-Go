...
| `text_xp.js` | `text_xp` | `text_xps` | `guild`, `member`, `xp`, `leavel` | XP by guild/member; rank by guild | none | `{guild:1,member:1}`, `{guild:1}` | xp/level stored as strings | text XP repository; XP admin upserts rows and XP reset deletes by guild/member or guild behind explicit gates |
