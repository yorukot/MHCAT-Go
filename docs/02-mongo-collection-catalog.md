...
| `birthday.js` | `birthday` | `birthdays` | `guild`, `user`, birthday date/time numbers, `allow` | user birthday by guild/user; list by guild; gated `增加`, `是否允許管理員設定`, `刪除`, and `生日列表` now use this collection | none | `{guild:1,user:1}`, `{guild:1}` | scheduler inactive; missing date parts; duplicate `{guild,user}` rows may exist before unique index audit | birthday profile/date CRUD; Go updates/deletes duplicate `{guild,user}` rows and creates no index |
