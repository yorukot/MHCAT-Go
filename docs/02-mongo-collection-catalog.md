...
| `work_user.js` | `work_user` | `work_users` | `guild`, `user`, `state`, `end_time`, `energi`, `get_coin` | work user by guild/user; active jobs by state/end_time; completed-work payout by non-idle state and `end_time` | payout resets `state` to `待業中`; lease-backed daily reset increments/clamps `energi` | `{guild:1,user:1}`, `{state:1,end_time:1}` | misspelled `energi`; duplicate work settings can repeat energy increments; payout crash between coin increment and state reset can repeat payout | work user state, recurring daily reset, and one-shot payout |
