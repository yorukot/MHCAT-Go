...
| `message_reactions` | `{guild:1, message:1, react:1}` | unique candidate after audit; non-unique lookup safe now | no | no | reaction-role lookup | reaction add/remove | emoji normalization/duplicates | apply non-unique lookup now; audit before unique | safe to drop |
| `btns` | `{guild:1, number:1}` | unique candidate after audit; non-unique lookup safe now | no | no | role-button interaction lookup | button components | duplicate button IDs | apply non-unique lookup now; audit before unique | safe to drop |
...
| `chats`, `chatgpts`, `chatgpt_gets`, `create_hours`, `good_webs`, `join_messages`, `leave_messages`, `loggings`, `tickets`, `verifications`, `work_sets` | `{guild:1}` | candidate after audit | no | no | guild singleton lookup | command/event config reads | duplicates/unknown live names | dry-run duplicate audit first | safe to drop |
...
| `guilds` | `{guild:1}` | unique candidate after audit; non-unique lookup safe now | no | no | announcement channel lookup | announcement send command | singleton duplicates/shared writers | apply non-unique lookup now; audit before unique | safe to drop |
...
| `btns` | `btns_guild_number_lookup` | `{guild:1,number:1}` | no | no | no | role button interaction lookup | non-unique index is duplicate-safe | explicit apply; remove before promoting the same-key unique index |
...
| `guilds` | `guilds_guild_lookup` | `{guild:1}` | no | no | no | announcement channel config lookup | non-unique index is duplicate-safe | explicit apply; remove before promoting the same-key unique index |
...
| `lotters` | `lotters_guild_id_lookup` | `{guild:1,id:1}` | no | no | no | lottery component lookup | non-unique index is duplicate-safe | explicit apply; remove before promoting the same-key unique index |
...
| `tickets` | `tickets_guild_lookup` | `{guild:1}` | no | no | no | ticket command/component config lookup | non-unique index is duplicate-safe | explicit apply; remove before promoting the same-key unique index |
