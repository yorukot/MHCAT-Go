...
| `gift_changes` | `gift_changes_time_guild` | `{time:1,guild:1}` | no | optional partial `time != 0` only after ADR | no | daily reset exclusion | partial semantics risk | dry-run only until data known |
...
| `polls` | `polls_guild_message` | `{guild:1,messageid:1}` | candidate | no | no | poll vote/result lookup | duplicate poll docs | audit, dry-run, explicit apply |
