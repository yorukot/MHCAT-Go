...
| `cron_sets` | `guild`, `id`, `cron`, `channel`, `message` | none | list reads by `guild`; active rows require a non-null/non-missing `cron`; direct and simplified setup write the exact legacy cron/message shape | setup inserts `{guild,id,channel,cron:null,message:null}` then direct or weekday/hour/minute completion sets `cron`/`message`; delete removes one `{guild,id}` row; list cleanup removes abandoned pending drafts | no | disable Go auto-notification config flag; Node.js can continue reading the same active rows | dashboard backup/export includes `cron_sets`; recurring scheduler ownership remains separate |
