...
| `loggings` | `guild`, `channel_id`, `message_update`, `message_delete`, `channel_update`, `member_voice_update` | none | config writes use exact legacy fields; message update/delete and channel update emitters read the same fields by `guild`; future voice emitters must do the same | `$set` legacy fields on all rows matching `guild`; `$setOnInsert` `guild` only when no rows match | no | disable Go logging-config, logging-message-event, and logging-channel-event flags; Node.js can continue reading the same fields | no known dashboard dependency, but log channel privacy should be reviewed |
