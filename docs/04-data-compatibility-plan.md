...
| Work | `energi`, `energy`, `time`, `end_time`, `state`, `get_coin` | scalar text plus integer fallback | exact tags | JavaScript-number coercion for number/string/null/boolean/non-finite values | null-as-zero; missing/malformed become NaN where legacy arithmetic applies | preserved | preserve misspelled `energi` and raw scalar display | preserve signed/decimal legacy arithmetic; audit active jobs | required |
...
| `cron_sets` | `guild`, `id`, `cron`, `channel`, `message` | none | list reads by `guild`; delivery applies the Mongoose-compatible scalar/Mixed boundary in [92-auto-notification.md](92-auto-notification.md), then reloads `{guild,id}` before each send | setup inserts `{guild,id,channel,cron:null,message:null}` then direct or weekday/hour/minute completion sets `cron`/`message`; delete removes one `{guild,id}` row; list cleanup removes abandoned pending drafts; delivery does not mutate rows | no | disable Go config and delivery flags; stop Go and confirm lease release before restoring Node `handler/cron.js` | dashboard backup/export includes `cron_sets`; delivery and config gates remain independent |
| `numbers` | `guild`, `parent`, all legacy stat channel/name String fields | none | exact guild command lookup and broad worker scan with Mongoose-compatible scalar coercion; preserve raw whitespace and null/missing/compound boundaries | base uses typed patch/upsert with 38 explicit nulls; optional counters patch one ID/name pair; delete removes exact guild duplicates; worker patches changed counter names | no | stop command/worker gates, reconcile in-flight names/counters, then restore Node exclusively | dashboard backup/export includes `numbers`; see [93-stats.md](93-stats.md) |
| `role_numbers` | `guild`, `channel`, `channel_name`, `role` | none | broad worker scan with Mongoose-compatible scalar coercion and raw whitespace | role creation removes exact `{guild,role}` duplicates then inserts one typed row; worker patches one arbitrary exact key match | no | stop Go worker/command ownership before restoring Node; preserve copied rows and old channels | dashboard backup/export includes `role_numbers`; see [93-stats.md](93-stats.md) |
