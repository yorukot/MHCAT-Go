...
| `loggings` | `guild`, `channel_id`, `message_update`, `message_delete`, `channel_update`, `member_voice_update` | none | read not required for the current config slice; future event emitters must read exact legacy fields | `$set` legacy fields on all rows matching `guild`; `$setOnInsert` `guild` only when no rows match | no | disable Go logging-config flag; Node.js can continue reading the same fields | no known dashboard dependency, but log channel privacy should be reviewed |
