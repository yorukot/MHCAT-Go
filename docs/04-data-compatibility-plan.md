...
| `mhcat_scheduler_locks` | none | `_id` equal to `lock_name`, `lock_name`, `owner`, `fence`, `expires_at`, `created_at`, `updated_at` | scheduler reads lease state only; expiry uses UTC instants | single-document acquire/renew/release with owner/fence checks; automatic-notification delivery owns `auto-notification-delivery`; release preserves the document and increments fence on next acquire | no legacy backfill | stop Go delivery, confirm release/expiry, then restore Node cron ownership | no dashboard dependency expected |
