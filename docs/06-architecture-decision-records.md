...
- Status: Accepted.
- Context: Accepted knowledge and blackjack games reserve both wagers, but the first Go handler only pruned stale sessions when a later component arrived. A stopped game could therefore retain both wagers indefinitely and leave active buttons visible. It also skipped the legacy 500-millisecond knowledge start, five-second knowledge reveal, and five-second higher/lower draw states. Timer callbacks, delayed transitions, and component handlers can reach the same state or terminal settlement concurrently.
- Decision: Give each active process-local game an explicit phase, strict legacy deadline, and monotonically increasing scheduled-event generation. Serialize state-changing components, delayed transitions, and timeout callbacks through session claims. Keep the knowledge scoring clock anchored to reveal start while separately recording question display time. Settle knowledge and blackjack forfeits according to the legacy response/turn state, delay higher/lower and final knowledge settlement until their visible transition completes, delete the session before editing Discord, and remove all components in timeout/reveal edits. Register the timer manager with interaction-runtime shutdown so pending callbacks are canceled and active callbacks finish before dependencies close.
...
- Consequences: Stopped games settle and update visibly during normal process uptime, stale transitions/timers cannot alter a newer phase, and a terminal button race produces one settlement. Knowledge uses the actual strict 21st interval tick, carries five elapsed seconds into each later question, and blackjack uses the strict 31st tick while preserving legacy visible timestamps. Higher/lower balances remain reserved during the five-second draw animation.
...
- Tests: deterministic acceptance/start/reveal/draw timing tests, carried knowledge-score tests, delayed/final payout and message tests, stale-generation tests, no-answer pot behavior, terminal-action race tests, fail-closed settlement tests, timer generation/cancellation tests, interaction-dispatcher shutdown tests, and race-detector coverage.
