...
- Ambiguous legacy IDs such as `announcement_yes`, `announcement_no`, `week_menu`, `hour_menu`, `min_menu`, and raw alphanumeric item/work/lottery IDs are rejected with `ErrAmbiguousID`. New auto-notification and birthday flows use feature-specific versioned state IDs instead.
