...
- Ambiguous legacy IDs such as `announcement_yes`, `announcement_no`, `week_menu`, `hour_menu`, `min_menu`, and generic raw alphanumeric item/work IDs are rejected with `ErrAmbiguousID`. Lottery routing accepts only the generated numeric `<digits>lotter<action?>` shape; new auto-notification and birthday flows use feature-specific versioned state IDs.
