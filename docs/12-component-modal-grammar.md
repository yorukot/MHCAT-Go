...
| Modal | `nal` + first input `anntag`/`ann*`; `mhcat:v1:announcement:submit:*` | `slashCommands/公告系統/announcement.js`, Go announcement handler | `events/modal.js`, Go typed router | tag, color, title, content | Announcement send | High in legacy; bounded in Go | Node routes both legacy and Go modals by first field; versioned ID owns new submissions | `announcement.modal.submit`; [contract](76-announcement.md) |
| Button | `announcement_yes`, `announcement_no`; `mhcat:v1:announcement:<confirm|cancel>:state=<id>` | `events/modal.js`, work setup, Go announcement handler | local collectors, Go typed router | confirmation choice or opaque state ID | Announcement and work confirmation | High in legacy; bounded in Go | raw IDs are ambiguous and rejected by Go; new state is owner/guild scoped and expires exactly at six seconds | `announcement.confirm/cancel`; [contract](76-announcement.md) |
| Modal/Button | `nal` + first field `ticketcolor`; `mhcat:v1:ticket:setup:*`; exact `tic`; exact `del` | `slashCommands/私人頻道/ticket.js`, `events/modal.js`, ticket events | legacy handlers and Go typed router | category/admin payload, panel fields, open/close action | Ticket/private channel | Medium in legacy; bounded in Go | exact first-field/ID routing; stale modal and side-effect compensation; owner close uses exact user-ID channel name | `ticket.setup/panel_submit/open/close`; [parity contract](74-ticket.md) |
...
| Modal/Button | `roleaddcontent<stamp><random>`; `<id>add`; `<id>delete` | `slashCommands/管理系統/releadd.js`, `events/modal.js` | `events/modal.js`, `events/btn.js`; Go typed router | JavaScript-shaped UTC+8 timestamp/random record ID and exact action suffix | Button role pickup | High in legacy; bounded in Go | legacy uses broad `includes("add")` / `includes("delete")`; Go validates numeric/decimal/exponent shape and exact suffix before DB lookup | `role_button.modal_submit`, `role_button.add/remove`; parity contract: [73-role-selection.md](73-role-selection.md) |
...
| Button | `<digits>lotter`, `<digits>lottersearch`, `<digits>lotterrestart`, `<digits>lotterstop` | disabled lottery creation path; old live messages possible | `events/btn.js`, Go lottery module | lottery ID and action suffix | Lottery | Low in Go | creation remains disabled; Go precisely parses and routes existing enter/search/reroll/stop buttons behind a separate gate | `lottery.enter/search/reroll/stop` |
...
| Select | `hour_menu`, `min_menu` | `slashCommands/生日系統/birthday.js` | local collectors | hour/min values | Birthday notification time | Medium | same IDs as cron wizard; Go emits owner-scoped `mhcat:v1:birthday:<hour|minute>:state=<id>` and does not accept raw IDs | `birthday.hour/min.select`; see [contract](78-birthday.md) |
...
The ID is generated by the Wave 4 custom ID encoder and must stay within Discord's 100-character `custom_id` limit. The modal creates `tickets` only after `ticketcolor`, `tickettitle`, and `ticketcontent` are validated, rejects a stale duplicate without sending, and removes only its own inserted row if panel delivery fails. Exact migration and rollback behavior is in the [ticket parity contract](74-ticket.md).
...
nal + firstTextInputId=ticketcolor
...
Go-generated role-button IDs intentionally retain the legacy UTC+8 timestamp plus JavaScript `Math.random() * 10000000000` decimal/exponent string shape. The parser accepts the documented legacy shape only; it does not route arbitrary IDs merely because they contain `add` or `delete`. Exact modal, panel, ownership, and stale-button behavior is in the [role-selection parity contract](73-role-selection.md).
...
- Ambiguous legacy IDs such as `announcement_yes`, `announcement_no`, `week_menu`, `hour_menu`, `min_menu`, and generic raw alphanumeric item/work IDs are rejected with `ErrAmbiguousID`. Lottery routing accepts only the generated numeric `<digits>lotter<action?>` shape; new auto-notification and birthday flows use feature-specific versioned state IDs. Birthday state is owner-scoped, process-local, and expires at five minutes as specified in [78-birthday.md](78-birthday.md).
