...
| ChatGPT service/worker | Bot has no OpenAI dependency. Legacy and gated Go runtimes write prompt state into `chatgpts`, wait ten seconds, then reread `chatgpts.message`; no local worker code found | `chats`, `chatgpts`, `chatgpt_gets` | `chats.guild/channel`; `chatgpts.guild/resid_c/resid_p/reply/message/time`; `chatgpt_gets.guild/price` | Bot read/write; external writer inferred but unconfirmed | High if the worker is absent or has undocumented semantics | Preserve patch-style handoff fields and request `time`. Go atomically debits/publishes and requires explicit worker/ownership confirmation; validate the real worker against [91-autochat-paid.md](91-autochat-paid.md). |
| Cron/worker | No separate cron worker repo found locally; cron runs inside legacy bot handlers; Go restores automatic notifications, daily reset, and work payout as separately gated recurring workers | `cron_sets`, `coins`, `gift_changes`, `work_sets`, `work_users`, `mhcat_scheduler_locks` | `cron`, `guild`, `channel`, `id`, loose `message`; `coin.today`; additive `coin.mhcat_work_payouts`; `work_user.energi`; `work_set.max_energy/get_energy`; `gift_change.time`; lease owner/fence/expiry | Bot internal read/write | High: Node does not participate in Go leases or payout markers, so overlapping owners can duplicate sends, resets, or payouts | Keep payloads and payout markers rollback-compatible. Every Go process needs a unique owner; corresponding Node handlers must be disabled before Go ownership. Auto-notification compatibility is canonical in [92-auto-notification.md](92-auto-notification.md); dashboard full-document backup/export should preserve the additive coin marker. |
...
- Confirm whether an external ChatGPT worker still exists, that it preserves request `time`, and that it consumes the documented six-field handoff within ten seconds before enabling Go paid auto-chat under [91-autochat-paid.md](91-autochat-paid.md).
...
External compatibility is resolved enough for guarded feature repositories that preserve the documented contracts. It is not resolved enough for schema cleanup, production index creation, or production paid ChatGPT enablement until the manual worker confirmation and live Mongo audit are complete.
