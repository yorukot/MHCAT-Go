...
| Cron/worker | No separate cron worker repo found locally; cron runs inside legacy bot `handler/cron.js` | `cron_sets`, `coins`, `gift_changes`, `work_sets`, `work_users` | `cron`, `guild`, `channel`, `id`, loose `message`; `coin.today`; `work_user.energi`; `work_set.max_energy/get_energy`; `gift_change.time` | Bot internal read/write | Medium: if Go externalizes scheduler, duplicate execution can send duplicate notifications or reset counters twice | Keep `cron_sets.message` as a loose Discord payload. Use one scheduler owner or a Mongo-backed lease before enabling scheduler in Go. |
