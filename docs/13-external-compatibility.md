...
| Cron/worker | No separate cron worker repo found locally; cron runs inside legacy bot `handler/cron.js`; Go restores only automatic-notification delivery as a recurring worker | `cron_sets`, `coins`, `gift_changes`, `work_sets`, `work_users`, `mhcat_scheduler_locks` | `cron`, `guild`, `channel`, `id`, loose `message`; `coin.today`; `work_user.energi`; `work_set.max_energy/get_energy`; `gift_change.time`; lease owner/fence/expiry | Bot internal read/write | High: Node does not participate in the Go lease, so overlapping owners duplicate sends; future reset/payout loops could duplicate mutations | Keep `cron_sets.message` loose and rollback-compatible. Go delivery must hold `auto-notification-delivery`, every Go process needs a unique owner, and Node `handler/cron.js` must be disabled before Go delivery is enabled. |
