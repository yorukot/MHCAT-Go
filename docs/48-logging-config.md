...
New config writes remain typed: `channel_id` is a BSON string and all toggles are BSON Booleans. Save updates every existing `{guild}` duplicate with `$set`; only a guild with no match receives an upserted row. No startup index is created, and `{guild:1}` remains a candidate only after duplicate audit.
