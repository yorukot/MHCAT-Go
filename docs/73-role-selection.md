...
Setup writes first update every duplicate logical row's `role`; only a key with no match is upserted. This keeps duplicate legacy rows aligned without assuming a unique index. Reaction delete removes every duplicate logical row. Runtime reads retain Mongo `findOne` semantics when pre-existing duplicates disagree.

The catalog lists candidate unique indexes `message_reactions_guild_message_react` and `btns_guild_number`, but the application creates no startup indexes. Do not apply either index until a full duplicate/malformed-scalar audit passes, explicit index application is reviewed, and Node/Go write ownership is exclusive. An index migration is optional for runtime correctness and must not be coupled to enabling the feature.
