...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_COIN_ADMIN:-false}" = "true" ] && [ "${MHCAT_FEATURE_ECONOMY_COIN_ADMIN_ENABLED:-false}" != "true" ]; then
  echo "refusing apply: economy coin-admin command apply requires MHCAT_FEATURE_ECONOMY_COIN_ADMIN_ENABLED=true for staging runtime parity" >&2
  exit 1
fi
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_COIN_RANK:-false}" = "true" ] && [ "${MHCAT_FEATURE_ECONOMY_COIN_RANK_ENABLED:-false}" != "true" ]; then
  echo "refusing apply: economy coin-rank command apply requires MHCAT_FEATURE_ECONOMY_COIN_RANK_ENABLED=true for staging runtime parity" >&2
  exit 1
fi
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_COIN_RESET:-false}" = "true" ] && [ "${MHCAT_FEATURE_ECONOMY_COIN_RESET_ENABLED:-false}" != "true" ]; then
  echo "refusing apply: economy coin-reset command apply requires MHCAT_FEATURE_ECONOMY_COIN_RESET_ENABLED=true for staging runtime parity" >&2
  exit 1
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_GAME:-false}" = "true" ] && [ "${MHCAT_FEATURE_ECONOMY_GAME_ENABLED:-false}" != "true" ]; then
  echo "refusing apply: economy game command apply requires MHCAT_FEATURE_ECONOMY_GAME_ENABLED=true for staging runtime parity" >&2
  exit 1
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_WARNING_REMOVAL:-false}" = "true" ] && [ "${MHCAT_FEATURE_WARNING_REMOVAL_ENABLED:-false}" != "true" ]; then
  echo "refusing apply: warning-removal commands apply requires MHCAT_FEATURE_WARNING_REMOVAL_ENABLED=true for staging runtime parity" >&2
  exit 1
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_GACHA_DRAW:-false}" = "true" ] && [ "${MHCAT_FEATURE_GACHA_DRAW_ENABLED:-false}" != "true" ]; then
  echo "refusing apply: gacha draw command apply requires MHCAT_FEATURE_GACHA_DRAW_ENABLED=true for staging runtime parity" >&2
  exit 1
fi
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_CREATE:-false}" = "true" ] && [ "${MHCAT_FEATURE_GACHA_PRIZE_CREATE_ENABLED:-false}" != "true" ]; then
  echo "refusing apply: gacha prize-create command apply requires MHCAT_FEATURE_GACHA_PRIZE_CREATE_ENABLED=true for staging runtime parity" >&2
  exit 1
fi
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_EDIT:-false}" = "true" ] && [ "${MHCAT_FEATURE_GACHA_PRIZE_EDIT_ENABLED:-false}" != "true" ]; then
  echo "refusing apply: gacha prize-edit command apply requires MHCAT_FEATURE_GACHA_PRIZE_EDIT_ENABLED=true for staging runtime parity" >&2
  exit 1
fi
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_DELETE:-false}" = "true" ] && [ "${MHCAT_FEATURE_GACHA_PRIZE_DELETE_ENABLED:-false}" != "true" ]; then
  echo "refusing apply: gacha prize-delete command apply requires MHCAT_FEATURE_GACHA_PRIZE_DELETE_ENABLED=true for staging runtime parity" >&2
  exit 1
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_STATS_ROLE_COUNT:-false}" = "true" ] && [ "${MHCAT_FEATURE_STATS_ROLE_COUNT_ENABLED:-false}" != "true" ]; then
  echo "refusing apply: stats role-count command apply requires MHCAT_FEATURE_STATS_ROLE_COUNT_ENABLED=true for staging runtime parity" >&2
  exit 1
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_XP_ROLE_CONFIG:-false}" = "true" ] && [ "${MHCAT_FEATURE_XP_ROLE_CONFIG_ENABLED:-false}" != "true" ]; then
  echo "refusing apply: XP role config commands apply requires MHCAT_FEATURE_XP_ROLE_CONFIG_ENABLED=true for staging runtime parity" >&2
  exit 1
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_XP_ADMIN:-false}" = "true" ] && [ "${MHCAT_FEATURE_XP_ADMIN_ENABLED:-false}" != "true" ]; then
  echo "refusing apply: XP admin command apply requires MHCAT_FEATURE_XP_ADMIN_ENABLED=true for staging runtime parity" >&2
  exit 1
fi
...
if [ "${MHCAT_FEATURE_XP_RESET_ENABLED:-false}" = "true" ] && [ "${MHCAT_DISCORD_GUILD_MESSAGES_INTENT:-false}" != "true" ]; then
  echo "refusing apply: XP reset runtime requires MHCAT_DISCORD_GUILD_MESSAGES_INTENT=true for staging confirmation parity" >&2
  exit 1
fi
if [ "${MHCAT_FEATURE_XP_RESET_ENABLED:-false}" = "true" ] && [ "${MHCAT_DISCORD_MESSAGE_CONTENT_INTENT:-false}" != "true" ]; then
  echo "refusing apply: XP reset runtime requires MHCAT_DISCORD_MESSAGE_CONTENT_INTENT=true for staging confirmation parity" >&2
  exit 1
fi
if [ "${MHCAT_FEATURE_ECONOMY_COIN_RESET_ENABLED:-false}" = "true" ] && [ "${MHCAT_DISCORD_ENABLE_GATEWAY:-false}" != "true" ]; then
  echo "refusing apply: economy coin-reset runtime requires MHCAT_DISCORD_ENABLE_GATEWAY=true for staging confirmation parity" >&2
  exit 1
fi
if [ "${MHCAT_FEATURE_ECONOMY_COIN_RESET_ENABLED:-false}" = "true" ] && [ "${MHCAT_DISCORD_GUILD_MESSAGES_INTENT:-false}" != "true" ]; then
  echo "refusing apply: economy coin-reset runtime requires MHCAT_DISCORD_GUILD_MESSAGES_INTENT=true for staging confirmation parity" >&2
  exit 1
fi
if [ "${MHCAT_FEATURE_ECONOMY_COIN_RESET_ENABLED:-false}" = "true" ] && [ "${MHCAT_DISCORD_MESSAGE_CONTENT_INTENT:-false}" != "true" ]; then
  echo "refusing apply: economy coin-reset runtime requires MHCAT_DISCORD_MESSAGE_CONTENT_INTENT=true for staging confirmation parity" >&2
  exit 1
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_VOICE_ROOM_LOCK:-false}" = "true" ] && [ "${MHCAT_FEATURE_VOICE_ROOM_LOCK_ENABLED:-false}" != "true" ]; then
  echo "refusing apply: voice-room lock command apply requires MHCAT_FEATURE_VOICE_ROOM_LOCK_ENABLED=true for staging runtime parity" >&2
  exit 1
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_COIN_ADMIN:-false}" = "true" ]; then
  echo "staging command sync apply: including economy coin-admin command" >&2
else
  echo "staging command sync apply: economy coin-admin command is excluded" >&2
fi
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_COIN_RANK:-false}" = "true" ]; then
  echo "staging command sync apply: including economy coin-rank command" >&2
else
  echo "staging command sync apply: economy coin-rank command is excluded" >&2
fi
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_COIN_RESET:-false}" = "true" ]; then
  echo "staging command sync apply: including economy coin-reset command 代幣重製" >&2
else
  echo "staging command sync apply: economy coin-reset command is excluded" >&2
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_GAME:-false}" = "true" ]; then
  echo "staging command sync apply: including economy game command" >&2
else
  echo "staging command sync apply: economy game command is excluded" >&2
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_ECONOMY_PROFILE:-false}" = "true" ]; then
  ...
  echo "staging command sync apply: economy profile command is excluded" >&2
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_WARNING_REMOVAL:-false}" = "true" ]; then
  echo "staging command sync apply: including warning-removal commands" >&2
else
  echo "staging command sync apply: warning-removal commands are excluded" >&2
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_GACHA_DRAW:-false}" = "true" ]; then
  echo "staging command sync apply: including gacha draw command" >&2
else
  echo "staging command sync apply: gacha draw command is excluded" >&2
fi
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_CREATE:-false}" = "true" ]; then
  echo "staging command sync apply: including gacha prize-create command" >&2
else
  echo "staging command sync apply: gacha prize-create command is excluded" >&2
fi
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_EDIT:-false}" = "true" ]; then
  echo "staging command sync apply: including gacha prize-edit command" >&2
else
  echo "staging command sync apply: gacha prize-edit command is excluded" >&2
fi
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_GACHA_PRIZE_DELETE:-false}" = "true" ]; then
  echo "staging command sync apply: including gacha prize-delete command" >&2
else
  echo "staging command sync apply: gacha prize-delete command is excluded" >&2
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_STATS_ROLE_COUNT:-false}" = "true" ]; then
  echo "staging command sync apply: including stats role-count command" >&2
else
  echo "staging command sync apply: stats role-count command is excluded" >&2
fi
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_STATS_DELETE:-false}" = "true" ]; then
  ...
  echo "staging command sync apply: stats delete command is excluded" >&2
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_XP_ROLE_CONFIG:-false}" = "true" ]; then
  echo "staging command sync apply: including XP role config commands" >&2
else
  echo "staging command sync apply: XP role config commands are excluded" >&2
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_XP_ADMIN:-false}" = "true" ]; then
  echo "staging command sync apply: including XP admin command" >&2
else
  echo "staging command sync apply: XP admin command is excluded" >&2
fi
...
if [ "${MHCAT_COMMAND_SYNC_INCLUDE_VOICE_ROOM_LOCK:-false}" = "true" ]; then
  echo "staging command sync apply: including voice-room lock command" >&2
else
  echo "staging command sync apply: voice-room lock command is excluded" >&2
fi
