...
- If voice-room lock smoke is enabled, seed a disposable `lock_channels` row for your current staging voice channel, run `/上鎖頻道` with and without `密碼`, verify legacy ephemeral success/errors and `lock_anser` replacement, then have another test user join the locked channel and verify prompt, disconnect, DM, modal answer, and `ok_people` append; verify no permission overwrite or dynamic voice room happened.
